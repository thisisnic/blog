...
Previously, I'd occasionally caught glimpses of things via social media, but I wanted a more methodical approach, and so decided to write [a GitHub Actions CRON job that does this](https://github.com/thisisnic/rwatcher/tree/main).  Now, when any of the packages on my list is updated, I receive an email that looks a little like this:
...
It has a name, and the schedule is set to run every day at 1am UTC.  The cron syntax is a bit weird, but there's a handy [crontab guru](https://crontab.guru/) that can help you figure out what you need to put in here.
...
Next, we set up the steps that we want to run.  The first step is to check out the repo, and get the hash of the changelog file in the repo.  This is saved to the `GITHUB_OUTPUT` environment variable, which is a file that is shared between all the steps in the job.
...
Next, I want to get the hash of the changelog file in the package repo.  I do this by downloading the file, and then getting the hash of the downloaded file.  This is also saved to the `GITHUB_OUTPUT` environment variable.
...
And that's it!  The full repository can be found [here](https://github.com/thisisnic/rwatcher).  

## Conclusion

I really enjoyed working on this and learning more about GitHub Actions.  This works reasonably well, though there are a few improvements that could be made:
...
Anyway, I'd love to hear your thoughts - how do you keep up to date with changes in R packages? Do you have any suggestions for improvements to this workflow?  Let me know!  Get in touch via [Mastodon](https://fosstodon.org/@thisisnic) or [LinkedIn](https://www.linkedin.com/in/nicolacrane).
